...
void weighted::dispenVaccine(int line, int vaccine, map<int, int> &population)
{
    float totalPopulation = 0;
    float weightedVacc;
    int distVaccine = 0;
    map<int, int>::iterator itr = population.begin();
    while (itr != population.end()) // calculating the total population of the simulation.
    {
        totalPopulation = totalPopulation + itr->second;
        ++itr;
    }
    map<int, int>::iterator ptr = population.begin();
    while (ptr != population.end()) // distribute the weighted vaccine: propotional to their region's population.
    {
        weightedVacc = ((ptr->second) / totalPopulation) * vaccine;
        distVaccine = distVaccine + weightedVacc;
        vaccDistribution.push_back(weightedVacc);
        ++ptr;
    }

    for (int i = 0; i < line - 1; i++) //distributing the remaning vaccine.
    {
        if (vaccine > distVaccine) //untill all the vaccine runs out.
        {
...
void weighted::SIRVsim(int numline, map<int, int> &mapPopn, int &infArea, int &infPeriod, int &conRate, map<int, int> &degCount)
{
    day = 0;
    totalInfected = 0;
    infRate = conRate;
...
void weighted::simulation(int numline, vector<vector<int>> adjList)
{
...
void weighted::print()
{
    ...
    if (storeMaxinfc.size() > 0)
    {
        ...
        cout << "Using the weighted distribution method, the peak number of infected was " << maxInfc << " on day " << maxDay << ". The outbreak ended on day " << day << " and the total number of infected individuals was " << storeTotInfc << "." << endl;
    }
    else
    {
        cout << "Using the weighted distribution method, the peak number of infected was " << 0 << " on day " << 0 << ". The outbreak ended on day " << 0 << " and the total number of infected individuals was " << 0 << "." << endl;
    }
}