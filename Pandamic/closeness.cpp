...
void closeness::initialization(int numline) //used to initialize the vacc distribution with 0;
{
...
void closeness::disVaccine(int numVac, map<int, int> &mapPopn, map<int, int> &degCount, map<int, double> &closenessCount)
{
    int minDeg;
    vector<int> checklist;
    checklist.push_back(0);
    for (int i = 0; i < closeness.size(); i++) // used to match the sorted vector of closeness and map of closness and then the refion is sorted.
    {
        bool check = false;
        map<int, double>::iterator ptr = closenessCount.begin();
        int num;
        while (ptr != closenessCount.end()) //untill the second element of map is not found which is eqivalent to the sorted ith vector of closeness.
        {
            if (closeness.at(i) == ptr->second) //
            {
...
void closeness::sorting(int numline, map<int, double> &closenessCount) //used to sort the vector of closennes in accending order.
{
    map<int, double> temp = closenessCount;
    map<int, double>::iterator ptr;
    for (ptr = temp.begin(); ptr != temp.end(); ++ptr)
    {
        closeness.push_back(ptr->second);
    }
    sort(closeness.begin(), closeness.end());
}

void closeness::SIRVsim(int numline, map<int, int> &mapPopn, int &infArea, int &infPeriod, int &conRate, map<int, int> &degCount)
{
    day = 0;
    totalInfected = 0;
    infRate = conRate;
...
void closeness::simulation(int numline, vector<vector<int>> adjList, map<int, double> &degCount)
{
...
void closeness::print()
{
    ...
    if (storeMaxinfc.size() > 0)
    {
        ...
        cout << "Using the closeness centrality distribution method, the peak number of infected was " << maxInfc << " on day " << maxDay << ". The outbreak ended on day " << day << " and the total number of infected individuals was " << storeTotInfc << "." << endl;
    }
    else
    {
        cout << "Using the closeness centrality distribution method, the peak number of infected was " << 0 << " on day " << 0 << ". The outbreak ended on day " << 0 << " and the total number of infected individuals was " << 0 << "." << endl;
    }
}